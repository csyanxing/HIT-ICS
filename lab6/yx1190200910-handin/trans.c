		...
		for (i = 0; i < M; i += 8)
		{
			for (j = 0; j < N; j += 8)
			{
				...
				for (k = j; k < j + 4; k++)
				{
					...
					t4 = B[k][i + 4]; t5 = B[k][i + 5];
					t6 = B[k][i + 6]; t7 = B[k][i + 7];
					B[k][i + 4] = t0; B[k][i + 5] = t1;
					B[k][i + 6] = t2; B[k][i + 7] = t3;
					B[k + 4][i] = t4; B[k + 4][i + 1] = t5;
					B[k + 4][i + 2] = t6; B[k + 4][i + 3] = t7;
				}
				for (k = i + 4; k < i + 8; k++)
				{
					t0 = A[k][j + 4]; t1 = A[k][j + 5];
					t2 = A[k][j + 6]; t3 = A[k][j + 7];
					B[j + 4][k] = t0; B[j + 5][k] = t1;
					B[j + 6][k] = t2; B[j + 7][k] = t3;
				}
			}
		}
	}else if (M == 61 && N == 67)
